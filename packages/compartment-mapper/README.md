...
const namespace = globalEval(script);
```
...
`makeScripts` accepts all the options of `makeArchive` and:
...
- `useEvaluate` (boolean, default `false`):
  ...
  each module function and use an evaluator function to rehydrate them.
...
const namespace = compartment.evaluate(functor)({
  require,
